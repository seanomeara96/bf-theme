...
import { scrollTop } from '../../emthemes-modez/theme-utils'; // papathemes-beautyfeatures
...
export default class ProductDetails {
    ...
    initTabScrollToViewport() {
        ...
        $('.productView-description .tab a', this.$scope).on('click', (event, { skipScroll } = {}) => {
            if (mediumMediaQueryList.matches) {
                if (!skipScroll) {
                    const $target = $($(event.currentTarget).attr('href'), this.$scope).first();
