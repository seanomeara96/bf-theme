...
export function autoExpandCategoryMenu(context) {
    // papathemes-beautyfeatures edited
    let $curMenuItem;
    ...
    if ($curMenuItem && $curMenuItem.length > 0) {
        ...
        $curMenuItem.parents('.navPage-childList, .navPage-subMenu, .navPages-rootMenu-list').prev('[data-collapsible]').each((i, el) => {
...
export function bindNavPagesCheckRightEdge(mediaQuery) {
    $('.navPages-item').hover(event => {
        ...
        $hoverEl.children('.navPage-subMenu').each((i, submenu) => {
            ...
            if ($submenu.offset().left + Math.floor($submenu.width()) > $(window).width()) {
                $submenu
                    .addClass('toLeft')
