...
export default function (context) {
    const $list = $('<ul/>')
        .addClass($('.navPages > .navPages-list').first().attr('class'))
        .addClass('navPages-list--fitNav')
        .append($('.navPages > .navPages-list').not('.navPages-list--user').children('.navPages-item').clone());

    if ($list.children().length === 0) {
        return;
    }

    $list.prependTo('.navPages');
